...
type instance Found Int n = 'Just 0
instance BaseCaseSearch Int z e where
  baseCaseSearch = const arbitrary

instance {-# INCOHERENT #-}
  ( TypeError
    (     ( 'Text "Could not find instance for (BaseCaseSearch ("
      ':<>: 'ShowType a
      ':<>: 'Text ") "
      ':<>: 'ShowType z
      ':<>: 'Text " ("
    ...
    ':$$: ( 'Text "If the type ("
      ':<>: 'ShowType a
      ':<>: 'Text ") is a type variable, you may be missing a (BaseCase ("
      ':<>: 'ShowType e
      ':<>: 'Text ")) constraint"
      )
    ':$$: ( 'Text "Otherwise, if it starts with a type constructor, \
                  \you may need to add an instance of BaseCaseSearch for it"
      )
    )
...
genericBCS = gbcs' (Proxy :: Proxy (GFound (Rep a) z))
